...
function [data,varss]=ALGOchoose(var,ngmar,aa,maxiteropt,bt,flag,stopindex,r,t,btmax,rho)
if(flag==0)
...
elseif(flag==3) 
[vars,loss,tr,bts,ats,reloss]=IBPG(var,ngmar,aa,maxiteropt,stopindex);
varss=vars;
lossdata=loss;
trdata=tr;
btss=bts;
...
elseif(flag==4) 
[vars,loss,tr,bts,~,reloss]=TITAN(var,ngmar,aa,maxiteropt,stopindex);
varss=vars;
lossdata=loss;
trdata=tr;
btss=bts;
...
elseif(flag==5)
[vars,loss,tr,bts,reloss]=ABPL(var,ngmar,aa,maxiteropt,bt,1,stopindex,t,btmax);  %%ABPL-random and ABPL+-random
varss=vars;
lossdata=loss;
trdata=tr;
btss=bts;
...
elseif(flag==6)
[vars,loss,tr,bts,reloss]=PGels(var,ngmar,aa,maxiteropt,stopindex,r);  %%ABPL-random and ABPL+-random
varss=vars;
lossdata=loss;
trdata=tr;
btss=bts;
...
elseif(flag==7) 
[vars,loss,tr,bts,reloss]=APGL(var,ngmar,aa,maxiteropt,stopindex,btmax,rho);
varss=vars;
lossdata=loss;
trdata=tr;
btss=bts;
